...
class CULDistributionDistance(MetricCallback):
    ...
    def __init__(
        ...
        **Args:**
        ...
        self.distribution_distance_csv_path: str = os.path.join(
            save_dir, distribution_distance_csv_name
        )
        ...
        if distribution_distance_plot_name:
            self.distribution_distance_plot_path: str = os.path.join(
                save_dir, distribution_distance_plot_name
            )
        ...
        r"""The scope to compute average DD over tasks."""

        self.average_task_ids: list[int] = []
        r"""Task IDs used to compute the average DD. Defaults to unlearned tasks if available; otherwise all eval tasks."""
    ...
    def sanity_check(self) -> None:
        ...
        if self.average_scope not in ["all", "remaining", "unlearned"]:
            raise ValueError(
                f"Invalid average_scope: {self.average_scope} in `CULDistributionDistance`. Must be one of 'all', 'remaining', or 'unlearned'."
            )
    ...
    def on_test_start(
        ...
        eval_task_ids = pl_module.dd_eval_task_ids
    ...
    def on_test_batch_end(
        ...
        **Args:**
        ...
        self.distribution_distance[test_task_id].update(distance_batch, batch_size)
    ...
    def update_distribution_distance_to_csv(
        ...
        **Args:**
        ...
        eval_task_ids = list(self.distribution_distance.keys())
        average_col_name = f"average_distribution_distance_on_{self.average_scope}"
        ...
        # write to the columns and calculate the average distribution distance over selected tasks
        average_distribution_distance_over_tasks = MeanMetric().to(
            device=next(iter(self.distribution_distance.values())).device
        )
        for task_id in eval_task_ids:
            distance = self.distribution_distance[task_id].compute().item()
            new_line[f"unlearning_test_on_task_{task_id}"] = distance
            if task_id in self.average_task_ids:
                average_distribution_distance_over_tasks(distance)
        average_distribution_distance = (
            average_distribution_distance_over_tasks.compute().item()
        )
        new_line[average_col_name] = average_distribution_distance
    ...
    def plot_distribution_distance_from_csv(
        ...
        **Args:**
        ...
        # Build the distribution distance matrix
        distance_matrix = data[test_task_cols].values

        # plot the distribution distance matrix
        fig, ax = plt.subplots(
            figsize=(2 * num_tasks, 2 * num_rows)
        )  # adaptive figure size
        vmin = float(distance_matrix.min())
        vmax = float(distance_matrix.max())
        cax = ax.imshow(
            distance_matrix,
