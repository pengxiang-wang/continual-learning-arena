...
class ParameterChangeReg(nn.Module):
    ...
    It promotes the target set of parameters $\theta = {\theta_i}_i$ not changing too much from another set of parameters $\theta^\star = {\theta^\star_i}_i$. The parameter distance here is $L^2$ distance. The regularisation can be parameter-wise weighted, i.e. $w_i$ in the formula.
