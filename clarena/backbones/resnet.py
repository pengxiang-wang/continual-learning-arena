...
class ResNetBlockSmall(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        **Returns:**
        - **output** (`Tensor`): the output feature maps.
        """
        identity = (
...
class ResNetBlockLarge(nn.Module):
...
class ResNetBase(CLBackbone):
    ...
    def forward(
        ...
        **Returns:**
        ...
        feature = x.view(batch_size, -1)  # flatten before going through heads
...
class HATMaskResNetBlockSmall(HATMaskBackbone):
    ...
    def __init__(
        ...
        **Args:**
        ...
        """
        super().__init__(output_dim=None, gate=gate)
        ...
        # construct the 1st weighted convolutional layer and attached layers (activation, etc)
        layer_input_channels = preceding_output_channels  # the input channels of the 1st convolutional layer, which receive the output channels of the preceding module
        layer_output_channels = (
            input_channels  # the output channels of the 1st convolutional layer
        )
        ...
        )  # store the full layer name to be masked
        self.task_embedding_t[self.full_1st_layer_name] = nn.Embedding(
            num_embeddings=1, embedding_dim=layer_output_channels
        )  # construct the task embedding over the weighted convolutional layer. It is channel-wise
        if self.activation:
            self.conv_activation1 = activation_layer()  # construct the activation layer
            r"""The activation layer after the 1st weighted convolutional layer. """

        # construct the 2nd weighted convolutional layer and attached layers (activation, etc)
        layer_input_channels = input_channels  # the input channels of the 2nd convolutional layer, which is `input_channels`, the same as the output channels of the 1st convolutional layer
        layer_output_channels = (
            input_channels * 1
        )  # the output channels of the 2nd convolutional layer, which is the same as the input channels (without expansion)
    ...
    def forward(
        ...
        **Returns:**
        ...
        x = x * mask[self.full_1st_layer_name].view(
            1, -1, 1, 1
        ...
        x = x + identity
        x = x * mask[self.full_2nd_layer_name].view(
            1, -1, 1, 1
...
class HATMaskResNetBlockLarge(HATMaskBackbone):
    ...
    def __init__(
        ...
        **Args:**
        ...
        self.masked_layer_names.append(
            self.full_2nd_layer_name
        )  # store the full layer name to be masked. It is channel-wise
        self.task_embedding_t[self.full_2nd_layer_name] = nn.Embedding(
            num_embeddings=1, embedding_dim=layer_output_channels
        )  # construct the task embedding over the weighted convolutional layer
        if self.activation:
            self.conv_activation2 = activation_layer()  # construct the activation layer
            r"""The activation layer after the 2nd weighted convolutional layer. """

        # construct the 3rd weighted convolutional layer and attached layers (activation, etc)
        layer_input_channels = (
            input_channels * 1
        )  # the input channels of the 2nd convolutional layer, which is `input_channels * 1`, the same as the output channels of the 1st convolutional layer
        layer_output_channels = (
    ...
    def forward(
        ...
        **Returns:**
        ...
        x = x + identity
        x = (x * mask[self.full_3rd_layer_name]).view(
            1, -1, 1, 1
...
class HATMaskResNetBase(HATMaskBackbone):
    ...
    def __init__(
        ...
        **Args:**
        ...
        r"""The 1st weight convolutional layer of the entire ResNet. It  is always with fixed kernel size 7x7, stride 2, and padding 3. """
        self.masked_layer_names.append("conv1")  # collect the layer name to be masked
        self.task_embedding_t["conv1"] = nn.Embedding(
            num_embeddings=1, embedding_dim=layer_output_channels
        )  # construct the task embedding over the weighted convolutional layers. It is channel-wise
        if self.activation:
            self.conv_activation1 = (
                activation_layer()
            )  # construct the 1st activation layer
            r"""The activation layer after the 1st weighted convolutional layer. """

        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  #
        r"""The max pooling layer which is laid in between 1st and 2nd convolutional layers with kernel size 3x3, stride 2. """

        # construct the 2nd convolutional layer with multiple blocks, and its attached layers (activation, etc)
        self.conv2x = self._multiple_blocks(
            layer_name="conv2x",
            building_block_type=building_block_type,
            building_block_num=building_block_nums[0],
        ...
        # construct the 3rd convolutional layer with multiple blocks, and its attached layers (activation, etc)
        self.conv3x = self._multiple_blocks(
            layer_name="conv3x",
            building_block_type=building_block_type,
            building_block_num=building_block_nums[1],
        ...
        # construct the 4th convolutional layer with multiple blocks, and its attached layers (activation, etc)
        self.conv4x = self._multiple_blocks(
            layer_name="conv4x",
            building_block_type=building_block_type,
            building_block_num=building_block_nums[2],
        ...
        # construct the 5th convolutional layer with multiple blocks, and its attached layers (activation, etc)
        self.conv5x = self._multiple_blocks(
            layer_name="conv5x",
            building_block_type=building_block_type,
            building_block_num=building_block_nums[3],
    ...
    def _multiple_blocks(
        ...
        **Return:**
        - **layer** (`nn.Sequential`): the constructed layer consisting of multiple building blocks.
        - **masked_layer_names** (`list[str]`): the list of full layer names to be masked in the blocks.
        """
        ...
        for block_idx in range(building_block_num):
            ...
            self.masked_layer_names += layer[
                -1
            ].masked_layer_names  # collect layer names to be masked in the blocks and add to the entire network
            self.task_embedding_t.update(
                layer[-1].task_embedding_t
            )  # collect task embeddings in the blocks and add to the entire network
