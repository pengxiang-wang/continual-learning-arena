r"""
The submodule in `stl_datasets` for USPS dataset.
"""
...
from torchvision.datasets import USPS as USPSRaw
...
class USPS(STLDatasetFromRaw):
    r"""USPS dataset. The [USPS dataset](https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/multiclass.html#usps) is a collection of handwritten digits. It consists of 9,298 handwritten digit images (10 classes), each 16x16 grayscale image."""

    original_dataset_python_class: type[Dataset] = USPSRaw
    ...
    def prepare_data(self) -> None:
        ...
        USPSRaw(root=self.root, train=True, download=True)
        USPSRaw(root=self.root, train=False, download=True)

        pylogger.debug(
            "The original USPS dataset has been downloaded to %s.",
            self.root,
        )
