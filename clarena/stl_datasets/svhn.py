r"""
The submodule in `stl_datasets` for SVHN dataset.
"""
...
from torchvision.datasets import SVHN as SVHNRaw
...
class SVHN(STLDatasetFromRaw):
    r"""SVHN dataset. The [SVHN dataset](http://ufldl.stanford.edu/housenumbers/) is a collection of street view house number images. It consists 73,257 training and 26,032 test images of 10 classes, each 32x32 color image."""

    original_dataset_python_class: type[Dataset] = SVHNRaw
    ...
    def __init__(
        ...
        r"""
        **Args:**
        - **root** (`str`): the root directory where the original SVHN data 'SVHN/' live.
    ...
    def prepare_data(self) -> None:
        ...
        SVHNRaw(root=self.root, split="train", download=True)
        SVHNRaw(root=self.root, split="test", download=True)

        pylogger.debug(
            "The original SVHN dataset has been downloaded to %s.", self.root
        )
