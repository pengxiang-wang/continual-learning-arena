r"""
The submodule in `stl_datasets` for Oxford 102 Flower dataset.
"""
...
from torchvision.datasets import Flowers102 as Flowers102Raw
...
class Flowers102(STLDatasetFromRaw):
    r"""Oxford 102 Flower dataset. The [Oxford 102 Flower dataset](https://www.robots.ox.ac.uk/~vgg/data/flowers/102/) is a collection of flower pictures. It consists of 8,189 images of 102 kinds of flowers (classes), each color image."""

    original_dataset_python_class: type[Dataset] = Flowers102Raw
    ...
    def prepare_data(self) -> None:
        r"""Download the original Oxford 102 Flower dataset if haven't."""

        Flowers102Raw(root=self.root, split="train", download=True)
        Flowers102Raw(root=self.root, split="val", download=True)
        Flowers102Raw(root=self.root, split="test", download=True)

        pylogger.debug(
            "The original Oxford 102 Flower dataset has been downloaded to %s.",
            self.root,
        )
