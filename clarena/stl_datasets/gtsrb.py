...
from torchvision.datasets import GTSRB as GTSRBRaw
...
class GTSRB(STLDatasetFromRaw):
    r"""GTSRB dataset. The [GTSRB dataset](http://yann.lecun.com/exdb/mnist/) is a collection of traffic sign images. It consists of 51,839 images of 43 different traffic signs (classes), each color image."""

    original_dataset_python_class: type[Dataset] = GTSRBRaw
    ...
    def prepare_data(self) -> None:
        ...
        GTSRBRaw(root=self.root, split="train", download=True)
        GTSRBRaw(root=self.root, split="test", download=True)

        pylogger.debug(
            "The original GTSRB dataset has been downloaded to %s.", self.root
        )
    ...
    def train_and_val_dataset(self) -> tuple[Dataset, Dataset]:
        ...
        **Returns:**
        ...
        """
        dataset_train_and_val = GTSRBRaw(
            root=self.root,
            split="train",
