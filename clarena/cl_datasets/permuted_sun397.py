...
class PermutedSUN397(CLPermutedDataset):
    r"""Permuted SUN397 dataset. The [original SUN397 dataset](https://vision.princeton.edu/projects/2010/SUN) is a collection of 397 categories of scenes. It consists of 108,754 images in 397 classes."""

    original_dataset_python_class: type[Dataset] = SUN397
    ...
    def __init__(
        ...
        r"""Initialise the Permuted SUN397 dataset object providing the root where data files live.
    ...
    def prepare_data(self) -> None:
        r"""Download the original SUN397 dataset if haven't."""

        if self.task_id != 1:
            return  # download all original datasets only at the beginning of first task

        SUN397(root=self.root_t, download=True)

        pylogger.debug(
            "The original SUN397 dataset has been downloaded to %s.", self.root_t
        )
