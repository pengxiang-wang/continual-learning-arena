...
from clarena.cl_datasets.original.notmnist import NotMNIST

# always get logger for built-in logging in each module
pylogger = logging.getLogger(__name__)


class PermutedNotMNIST(CLPermutedDataset):
    r"""Permuted NotMNIST dataset. The [original NotMNIST dataset](https://yaroslavvb.blogspot.com/2011/09/notmnist-dataset.html) is a collection of letters (including A-J). It consists of around 19,000 28x28 grayscale images in 10 classes."""

    original_dataset_python_class: type[Dataset] = NotMNIST
    ...
    def __init__(
        ...
        r"""Initialise the Permuted NotMNIST dataset object providing the root where data files live.
    ...
    def prepare_data(self) -> None:
        ...
        if self.task_id == 1:
            ...
            NotMNIST(root=self.root_t, train=True, download=True)
            NotMNIST(root=self.root_t, train=False, download=True)

            pylogger.debug(
                "The original NotMNIST dataset has been downloaded to %s.", self.root_t
            )
    ...
    def train_and_val_dataset(self) -> tuple[Dataset, Dataset]:
        ...
        **Returns:**
        ...
        """
        dataset_train_and_val = NotMNIST(
            root=self.root_t,
            train=True,
