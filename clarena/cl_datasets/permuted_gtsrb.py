...
class PermutedGTSRB(CLPermutedDataset):
    r"""Permuted GTSRB dataset. The [original GTSRB dataset](http://yann.lecun.com/exdb/mnist/) is a collection of handwritten digits. It consists of 70,000 28x28 B&W images in 10 classes (correspond to 10 digits), with 7000 images per class. There are 60,000 training examples and 10,000 test examples."""

    original_dataset_python_class: type[Dataset] = GTSRB
    ...
    def __init__(
        ...
        r"""Initialise the Permuted GTSRB dataset object providing the root where data files live.
    ...
    def prepare_data(self) -> None:
        r"""Download the original GTSRB dataset if haven't."""

        if self.task_id != 1:
            return  # download all original datasets only at the beginning of first task

        GTSRB(root=self.root_t, split="train", download=True)
        GTSRB(root=self.root_t, split="test", download=True)

        pylogger.debug(
            "The original GTSRB dataset has been downloaded to %s.", self.root_t
        )
