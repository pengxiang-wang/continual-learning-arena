...
class PermutedPCAM(CLPermutedDataset):
    r"""Permuted PCAM dataset. The [original PCAM dataset](hthttps://github.com/basveeling/pcam) is a collection of medical images of breast cancer. It consists of 327,680 images in 2 classes."""

    original_dataset_python_class: type[Dataset] = PCAM
    ...
    def __init__(
        ...
        r"""Initialise the Permuted PCAM dataset object providing the root where data files live.
    ...
    def prepare_data(self) -> None:
        ...
        PCAM(root=self.root_t, split="train", download=True)
        PCAM(root=self.root_t, split="val", download=True)
        PCAM(root=self.root_t, split="test", download=True)

        pylogger.debug(
            "The original PCAM dataset has been downloaded to %s.", self.root_t
        )
