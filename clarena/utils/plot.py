...
__all__ = ["plot_ave_acc_from_csv", "plot_acc_metrix_from_csv", "plot_ave_loss_cls_from_csv", "plot_loss_cls_metrix_from_csv", "plot_hat_mask"]
...
def plot_acc_matrix_from_csv(csv_path: str, task_id: int, plot_path: str) -> None:
    ...
    **Args:**
    ...
    fig, ax = plt.subplots(figsize=(2*(task_id+1), 2*(task_id+1))) # adaptive figure size
    cax = ax.imshow(
    ...
    colorbar.ax.set_yticklabels([f"{tick:.2f}" for tick in yticks], fontsize=10+task_id) # adaptive font size
    ...
    ax.set_xticklabels(range(1, task_id + 1), fontsize=10+task_id) # adaptive font size
    ax.set_yticklabels(range(1, task_id + 1), fontsize=10+task_id) # adaptive font size
...
def plot_hat_mask(mask: dict[str, Tensor], plot_dir: str, task_id: int, step: int | None = None) -> None:
    ...
    **Args:**
    ...
    for module_name, m in mask.items():
        ...
        plt.colorbar()
        if step: 
            plot_name = f"{module_name}_task{task_id}_step{step}.png"
        else:
            plot_name = f"{module_name}_task{task_id}.png"
