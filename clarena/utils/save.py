...
def save_joint_test_acc_to_csv(
    test_acc_metric: dict[str, MeanMetricBatch],
    csv_path: str,
) -> None:
    r"""Save the test accuracy metrics of all tasks in joint learning to an CSV file. Used in `JLMetricsCallback`.

    **Args:**
    - **test_acc_metric** (`dict[str, MeanMetricBatch]`): classification accuracy of the test data of each task. Accumulated and calculated from the test batches.
    - **csv_path** (`str`): save the test metric to path. E.g. './outputs/expr_name/1970-01-01_00-00-00/results/acc.csv'.
    """
    all_task_ids = list(test_acc_metric.keys())
...
def save_joint_test_loss_cls_to_csv(
    test_loss_cls_metric: dict[str, MeanMetricBatch],
    csv_path: str,
) -> None:
    r"""Save the test classification loss metrics of all tasks in joint learning to an CSV file. Used in `JLMetricsCallback`.

    **Args:**
    - **test_loss_cls_metric** (`dict[str, MeanMetricBatch]`): classification loss of the test data of each task. Accumulated and calculated from the test batches.
    - **csv_path** (`str`): save the test metric to path. E.g. './outputs/expr_name/1970-01-01_00-00-00/results/loss_cls.csv'.
    """
    all_task_ids = list(test_loss_cls_metric.keys())
    ...
    for task_id in all_task_ids:
        ...
        new_line[f"test_on_task_{task_id}"] = acc
        average_classification_loss_over_tasks(acc)
    new_line["average_classification_loss"] = (
        average_classification_loss_over_tasks.compute().item()
    )
